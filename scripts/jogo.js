document.addEventListener('DOMContentLoaded', function() {
    ...
    function getBiomeFallbackColor(biome) {
      ...
      function initializeGrid(faseConfig) {
        ...
        faseConfig.fruits = faseConfig.fruits || [];
        faseConfig.obstaculos = faseConfig.obstaculos || [];
        faseConfig.accessibleCells = faseConfig.accessibleCells || [];
        ...
        for (let y = 1; y < faseConfig.grid.height - 1; y++) { // Evitar bordas superior e inferior
            for (let x = 1; x < faseConfig.grid.width - 1; x++) { // Evitar bordas laterais
                ...
                const isAccessible = faseConfig.accessibleCells.some(pos => pos.x === x && pos.y === y);
                const isObstacle = faseConfig.obstaculos.some(obs => obs.x === x && obs.y === y);
                const isStart = (x === faseConfig.inicio.x && y === faseConfig.inicio.y);
                const isEnd = (x === faseConfig.fim.x && y === faseConfig.fim.y);
                const isFruit = faseConfig.fruits.some(fruit => fruit.x === x && fruit.y === y);

                // Verificar se a célula é bloqueada e não é um obstáculo original
                const isOriginallyBlocked = !isAccessible && !isObstacle && !isStart && !isEnd && !isFruit;
        ...
        for (let y = 0; y < faseConfig.grid.height; y++) {
            ...
            for (let x = 0; x < faseConfig.grid.width; x++) {
                ...
                if (x === faseConfig.inicio.x && y === faseConfig.inicio.y) {
                    ...
                    cellType = 'end';
                } else if (faseConfig.fruits.some(fruit => fruit.x === x && fruit.y === y)) {
                    cellType = 'fruit';
    ...
    function moveCharacter(direction) {
        return new Promise(resolve => {
            ...
            // CORREÇÃO: Verificar se a posição existe no grid
            if (!gameState.grid[newY] || !gameState.grid[newY][newX]) {
                resolve('blocked');
                return;
            }
