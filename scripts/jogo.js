document.addEventListener("DOMContentLoaded", function () {
  ...
  function playGameSound(soundName) {
    if (window.soundManager && typeof window.soundManager.play === "function") {
      ...
      console.log("Som:", soundName); // Fallback para debug
    }
  }
  ...
  function showFruitInfoModal(biome) {
    const fruitInfo = getFruitInformation(biome);
    biomeModalTitle.textContent = `FRUTA - ${getFruitTitle(biome)}`;
    biomeInfoContent.innerHTML = fruitInfo;
    biomeInfoModal.style.display = "flex";
  }

  // Função auxiliar para obter o título da fruta
  function getFruitTitle(biome) {
    const titles = {
      atlantic: "CAMBUCÍ",
      amazon: "AÇAÍ",
      cerrado: "PEQUI",
    ...
    return titles[biome] || "FRUTA";
  }

  // Função para obter informações das frutas
  function getFruitInformation(biome) {
    const fruitData = {
      atlantic: {
        name: "CAMBUCÍ",
        scientificName: "Plinia edulis",
        description:
          "Fruta nativa da Mata Atlântica, conhecida por seu sabor doce e azedinho.",
        characteristics: [
          "🌿 Nativa da Mata Atlântica",
          "🍯 Sabor doce e levemente ácido",
          "💪 Rica em vitamina C e antioxidantes",
          "🌳 Árvore de médio porte, até 10 metros",
      ...
      amazon: {
        ...
        description:
          "Fruta tipicamente amazônica, super nutritiva e energética.",
        characteristics: [
          "🌴 Nativo da Amazônia",
          "💪 Super alimento energético",
          "🟣 Rico em antocianinas (antioxidantes)",
          "🌧️ Cresce em áreas alagadas",
          "🏋️‍♂️ Muito consumido por atletas",
        ],
      },
      cerrado: {
        name: "PEQUI",
        ...
        description:
          "Fruta símbolo do Cerrado, conhecida por seus espinhos e sabor marcante.",
        characteristics: [
          "🌵 Símbolo do Cerrado brasileiro",
          "⚠️ Cuidado com os espinhos internos!",
          "🎨 Polpa amarela intensa",
          "👃 Aroma e sabor marcantes",
          "🔥 Muito usado na culinária regional",
        ],
      },
      caatinga: {
        name: "CAJU",
        ...
        description:
          "Fruta tropical, onde a castanha é a verdadeira semente e o caju é o pseudofruto.",
        characteristics: [
          "🏜️ Adaptado ao clima semiárido",
          "🥜 A castanha é a verdadeira fruta",
      ...
      pampa: {
        ...
        description:
          "Fruta nativa do Pampa, pequena e saborosa, parente da jabuticaba.",
        characteristics: [
          "🌾 Nativo dos Pampas gaúchos",
          "🍇 Parente da jabuticaba",
          "⚫ Pequena e de cor roxa-escura",
          "😊 Sabor doce e suave",
          "🌿 Árvore ornamental",
        ],
      },
      pantanal: {
        ...
        description:
          "Palmeira típica do Pantanal, com frutos utilizados na alimentação e artesanato.",
        characteristics: [
          "🌊 Típica do Pantanal",
          "🥥 Fruto de palmeira",
          "🍞 Polpa farinácea",
          "🎨 Usada no artesanato local",
          "🐄 Alimento para o gado",
        ],
      },
    };

    const data = fruitData[biome] || fruitData["atlantic"];
  ...
  function addCellClickEvents() {
    ...
    document.querySelectorAll(".animal-obstacle, .fruit").forEach((cell) => {
      cell.replaceWith(cell.cloneNode(true));
    });

    // Adiciona os novos event listeners com delegation
    gridElement.addEventListener("click", function (event) {
      const animalCell = event.target.closest(".animal-obstacle");
      const fruitCell = event.target.closest(".fruit");

      if (animalCell) {
        console.log("Animal clicado!");
        showAnimalInfoModal(gameState.currentBiome);
      }

      if (fruitCell) {
        console.log("Fruta clicada!");
        showFruitInfoModal(gameState.currentBiome);
      }
    });

    // Marca visualmente os animais e frutas clicáveis
    document.querySelectorAll(".animal-obstacle").forEach((cell) => {
      cell.style.cursor = "pointer";
      cell.title = "Clique para saber mais sobre este animal";
    });

    document.querySelectorAll(".fruit").forEach((cell) => {
      cell.style.cursor = "pointer";
      cell.title = "Clique para saber mais sobre esta fruta";
    });

    console.log(
      "Eventos de células configurados - Animais:",
      document.querySelectorAll(".animal-obstacle").length,
      "Frutas:",
      document.querySelectorAll(".fruit").length
    );
  }
  ...
  function initializeGrid(faseConfig) {
    ...
    gameState.totalItems = faseConfig.fruits ? faseConfig.fruits.length : 0;
    ...
    for (let y = 1; y < faseConfig.grid.height - 1; y++) {
      ...
      for (let x = 1; x < faseConfig.grid.width - 1; x++) {
        ...
        const isOriginallyBlocked =
          ...
          !faseConfig.fruits.some((fruit) => fruit.x === x && fruit.y === y);
  ...
  async function executeCommands() {
    ...
    for (let i = 0; i < gameState.commands.length; i++) {
      ...
      if (winStatus === "incomplete") {
        playGameSound("obstacle-hit");
        alert("VOCÊ CHEGOU AO FINAL MAS NÃO COLETOU TODAS AS FRUTAS");
        resetGame();
        runBtn.disabled = false;
        resetBtn.disabled = false;
        return;
  ...
  function updateCounters() {
    ...
    itemsCounter.textContent = `FRUTAS: ${gameState.itemsCollected}/${gameState.totalItems}`;
  }
