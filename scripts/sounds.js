...
class SoundManager {
    ...
    updateMuteButtons() {
        ...
        muteBtns.forEach(btn => {
            if (btn) {
                // Verificar se é o botão do jogo (com texto) ou dos menus (apenas ícone)
                if (btn.textContent.includes('SOM') || btn.textContent.includes('MUDO')) {
                    // Botão do jogo - com texto
                    btn.innerHTML = this.isMuted ? '🔇 MUDO' : '🔊 SOM';
                } else {
                    // Botão dos menus - apenas ícone
